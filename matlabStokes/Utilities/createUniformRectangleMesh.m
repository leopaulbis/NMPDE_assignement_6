function [X,T] = createUniformRectangleMesh(elem,degree,x1,x2,y1,y2,nx,ny)

nen=(degree+1).^2;
nx=degree*nx; ny=degree*ny;
[X,T] = CreaMalla(elem,nen,x1,x2,y1,y2,nx,ny);

function [X,T] = CreaMalla(elem,nen,x1,x2,y1,y2,nx,ny)
% [X,T] = CreaMalla_plus(elem,nen,x1,x2,y1,y2,nx,ny)
% Crea la topolog�a de una malla estructurada y uniforme
% en un dominio rectangular [x1,x2]X[y1,y2]
% Input:    elem: Tipo de elemento utilizado (0: cuadril�teros,   1: tri�ngulos)
%           nen: n�mero de nodos por elemento
%           x1,x2,y1,y2: Coordenadas de los v�rtices del dominio
